...
%% Obliczenia Offline
[S_data, Sz_data] = get_s_(D+1, F_C, F_H, F_D0, T_C, T_H, T_D, Tau_C, C, alpha, Hpp, Tpp, Tp, delay_u1, delay_steps);
...
y_zad(2000:end, 1) = 1.0*Hpp;
y_zad(500:end, 2) = 1.4*Tpp;
y_zad(3000:end, 2) = 1.2*Tpp;

F_D_seq = F_D0 * ones(N_steps, 1);
F_D_seq(6000:end) = 1.2 * F_D0; 
...
exportgraphics(gcf, "images/dmc_control_y.pdf", "ContentType", "vector");
...
grid on;
set(gcf, 'position', [10, 10, 1000, 700]);
exportgraphics(gcf, "images/dmc_control_u.pdf", "ContentType", "vector");