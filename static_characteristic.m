...
for i = 1:N
    ...
    for j = 1:N
        ...
        [~, S_nl] = ode15s(@(t, x) nonlinear_model(x,t,z,T_C,T_H,T_D,F_C,u,F_D,Tau_C,C,alpha), Ts, [H0; T0], options);
        [~, S_l] = ode15s(@(t, x) linear_model(x,t,z,T_C,T_H,T_D,F_C,u,F_D,Tau_C,C,alpha), Ts, [H0; T0], options);
        
        y_steady(i, j) = S_l(end, 1);
        y_steady_nonlinear(i, j) = S_nl(end, 1);
